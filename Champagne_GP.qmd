...
def discrepency_fn(alpha_, beta_, gamma_L, lambda_, f, r, mean_of=30):
...
```{python}
num_slice_updates = 10

all_slices = [
    np.linspace(0, 1, num_slice_updates, dtype=np.float64),  # alpha
...
for t in range(501):
    ...
    slice_var = [next_alpha, next_beta, next_gamma_L, next_lambda, next_f, next_r][
        t % 6
    ]
    if np.random.uniform() < 1 / 20 + np.exp(1 - t / 4):
...
for var in vars:
    ...
    variance = 30**2 * observation_noise_variance_champ.numpy()
    post_std = np.sqrt(variance)
    cdf_vals = tfd.Normal(mean, post_std).log_cdf(epsilon)
    likelihood_dict[var + "_synth_lik"] = cdf_vals
