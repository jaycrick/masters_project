...
for t in range(100):
    ...
    index_vals = np.append(
        ...
        np.array(
            [
                tfb.Sigmoid().forward(next_alpha).numpy(),
                tfb.Sigmoid().forward(next_beta).numpy(),
                constrain_positive.forward(next_gamma_L).numpy(),
                constrain_positive.forward(next_lambda).numpy(),
                constrain_positive.forward(next_f).numpy(),
                constrain_positive.forward(next_r).numpy(),
            ]
        ).reshape(1, -1),
        axis=0,
    )
    ...
    def optimize():
        with tf.GradientTape() as tape:
            ...
            loss = tf.squeeze(
                mean_t
                - np.sqrt(2 * np.log((t + 1) ** (6 / 2 + 2) * np.pi**2 / (3 * 0.1)))
                * std_t
            )
        grads = tape.gradient(loss, next_vars)
        Adam_optim.apply_gradients(zip(grads, next_vars))
