...
min_obs = tf.Variable(0,dtype=np.float64, name = "min_obs", shape=())
...
for t in range(2):
    ...
    for i in range(len(next_vars)):
        if i < 2:
            next_vars[i].assign(tfb.Sigmoid().forward(min_index[i]))
        else:
            next_vars[i].assign(constrain_positive(min_index[i]))
