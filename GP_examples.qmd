...
for ell in [0.5, 1, 2]:
    ...
    plt.xlabel("x")
    plt.ylabel("f(x)")
    plt.title(r"Function estimation with $\ell = $" + str(ell))
    plt.savefig("write_up/images/exponentiated_ell_" + str(int(ell*10)) + "_tenths.pdf")
    plt.show()
```

## Changing length parameter
```{python}
...
for nu in [0.5, 1.5, 2.5]:
  ...
  plt.xlabel("x")
  plt.ylabel("f(x)")
  plt.title(r"Matern kernel estimation with $\nu = $" + str(nu))
...
index_vals = np.expand_dims(  # this makes the size of the sample (3,1)
  my_seed.uniform(low=-1.0, high=1.0, size=5), axis=-1  # last axis
)
...
for ell in [0.3, 0.6, 1, 2]:
  ...
  plt.ylabel("f(x)")
  plt.title(r"Function estimation with $\ell = $" + str(ell))
  plt.savefig("write_up/images/cub_GP_ell_" + str(ell*10) + "_tenths.pdf")
  plt.show()
```
...
for sigma in [0.3, 0.6, 1, 2]:
  ...
  plt.ylabel("f(x)")
  plt.title(r"Function estimation with $\sigma = $" + str(sigma))
  # plt.savefig("write_up/images/cub_GP_ell_" + str(sigma*10) "_tenths.pdf")
  plt.show()
```
