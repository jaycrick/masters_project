...
outputs <- ode(y = state, times = times, func = SIS_function, parms = parameters) %>%
  as_tibble() %>%
  mutate(s = as.double(s),
         i = as.double(i),
         time = as.double(time))

long_outputs <- pivot_longer(outputs, c(s, i), names_to = "state", values_to = "proportions")

I_t = function(t, b = SIS_beta, g = SIS_gamma, population = pop_size, initial_infections = 1) {
  if (b == g){
...
binom_rho_accept = function(x, y, cases = binom_estimate, population = pop_size) {
  log_likihood_ratio = dbinom(cases, (population - round(I_t(sample_day, g = y), 0)), SIS_beta*I_t(sample_day, g = y)/population, log = T) + dgamma(y, 2, 6, log = T) -
                         dbinom(cases, (population - round(I_t(sample_day, g = x), 0)), SIS_beta*I_t(sample_day, g = x)/population, log = T) - dgamma(x, 2, 6, log = T)
...
binom_MH_tbl = tibble(Samples = binom_MH_samples)

gamma_dens = dgamma(0.225 + (1:999)/100000, 2, 6, log = T)

infecteds_list <- sapply(0.225 + (1:999)/100000, function(x) I_t(sample_day, g = x))

binom_infected_likelihoods <- sapply(infecteds_list, function(x) dbinom(binom_estimate, pop_size - round(x, 0), SIS_beta*x/pop_size, log = T))
...
pois_rho_accept = function(x, y, cases = binom_estimate, population = pop_size) {
  log_likihood_ratio = dpois(cases, SIS_beta*I_t(sample_day, g = y)*(population - I_t(sample_day, g = y))/population, log = T) + dgamma(y, 2, 6, log = T) -
    dpois(cases, SIS_beta*I_t(sample_day, g = x)*(population - I_t(sample_day, g = x))/population, log = T) - dgamma(x, 2, 6, log = T)
  likelihood_ratio = exp(log_likihood_ratio)
  min(likelihood_ratio, 1)
}
...
for (i in 2:no_samples){
  ...
  pois_MH_samples[i] = sample(c(last_sample, new_guess), 1, prob = c(1- accept_prob, accept_prob))
}

pois_MH_tbl = tibble(Samples = pois_MH_samples)

pois_infected_likelihoods <- sapply(infecteds_list, function(x) dpois(binom_estimate, SIS_beta*x*(pop_size - x)/pop_size, log = T))
