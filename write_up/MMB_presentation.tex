...
\begin{frame}
    ...
    \only<6>{\includegraphics[width=\textwidth]{cub_GP_err_10_iters.pdf}}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item What if we could 'predict' discrepency values we hadn't seen before?
              \only<2>{\item For parameters 'close' to parameters we've already tried it should be easy}
...
\begin{frame}
    \frametitle{Champagne Model Diagram}
    \begin{figure}[htbp]
        \centering
