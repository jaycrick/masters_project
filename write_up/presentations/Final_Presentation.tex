...
\begin{frame}
    \frametitle{Bayesian Acquisition}
    \begin{itemize}
        \only<1>{\item High expected $\mathcal{D}_\mathcal{GP}(\bm{\theta})$ with low
              variance = waste of time (and resources)
        \item Quantify this using a Bayesian acquisition function $A,$ and choose
              $\argmin_{\bm{\theta}}A(\bm\theta)$}
              \only<2>{\item \cite{gutmann_bayesian_2016} uses lower confidence bound
              $$A_\text{LCB}(\bm\theta) := \mu(\bm\theta) - \eta_t\sqrt{\mathrm{v}(\bm\theta)}$$ \begin{itemize}
                  \item $\mu(\bm\theta),$
                        $\mathrm{v}(\bm\theta)$ are posterior mean and variance
                  \item $\eta_t:= \sqrt{2\ln[t^{d/2+2}\pi^2/(3\epsilon)]},$
                        with $\epsilon\in(0,1)$
                  \item (Claim of theoretical guarantees = load of rubbish)
              \end{itemize}}
              \only<3>{\item  Expected information
              \begin{align*}
                  A_\text{EI}(\bm\theta)
                  := & \mathbb{E}(\min[\mathcal{D}_\mathcal{GP}(\bm\theta) - \mu_\text{min}, 0)]           \\
              ...
              \end{itemize}}
              \only<4>{\item Theoretical guarantees highly sensitive to choice of kernel}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item $L(\bm\theta | \mathbf{Y}_\text{obs}) \approx P(\mathcal{D}_{\mathcal{GP}}(\bm{\theta}) < \varepsilon)$
              (up to a proportion)
              \begin{figure}
                  \centering
                  \begin{tikzpicture}
                      \begin{axis}[
                              xlabel={$\mathcal{D}_\mathcal{GP}(\bm{\theta})$},
                              ylabel={Probability of Acceptance},
                              xmin=0, xmax=2,
                              ymin=0, ymax=1.1,
                              axis lines=left,
...
\begin{frame}
    \frametitle{Vivax Malaria}
    \begin{itemize}
        \item Has dormant liver stage on top of blood stage infection that can cause
              relapse.
    \end{itemize}
\end{frame}
    ...
    \begin{figure}[htbp]
        ...
        \begin{tikzpicture}[thick]
            ...
            \draw[->] (SL) edge[in = 170, out = 10, sloped] node [above]
                {$\alpha( 1 -\beta)(\lambda I_\mathrm{total} + \delta + f)$} (IL);
...
\begin{frame}
    ...
    \begin{align*}
        ...
        + \alpha(1-\beta)(\lambda I_\mathrm{total}                                      \\
          & + \delta)S_0
        ...
        + \alpha\beta fS_\mathrm{L}                                                     \\
          & + \gamma_\mathrm{L}S_\mathrm{L} + rI_0
    \end{align*}
\end{frame}

\begin{frame}
    \frametitle{Champagne Model Parameters}\begin{itemize}
        \item $\alpha:$ proportion of those infected but cleared of blood stage infections
              (through treatment)
        \item $\beta:$ a further proportion that are also cleared of liver stage parasites,
              given that they were also cleared of blood stage infection (radical cure)
...
\begin{frame}
    ...
    \only<2>{\includegraphics[width=\textwidth]{../../champagne_GP_images/initial_lambda_slice_log_discrep.pdf}}
    \only<3>{\includegraphics[width=\textwidth]{../../champagne_GP_images/lambda_slice_50_bolfi_updates_log_discrep.pdf}}
    \only<4>{\includegraphics[width=\textwidth]{../../champagne_GP_images/lambda_slice_200_bolfi_updates_log_discrep.pdf}}
    \only<5>{\includegraphics[width=\textwidth]{../../champagne_GP_images/lambda_slice_200_synth_likelihood.pdf}}
    \only<6>{\includegraphics[width=\textwidth]{../../champagne_GP_images/initial_f_slice_log_discrep.pdf}}
    \only<7>{\includegraphics[width=\textwidth]{../../champagne_GP_images/f_slice_200_bolfi_updates_log_discrep.pdf}}
    \only<8>{\includegraphics[width=\textwidth]{../../champagne_GP_images/f_slice_200_synth_likelihood.pdf}}
\end{frame}
