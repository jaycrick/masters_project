...
\begin{frame}
    ...
    \begin{itemize}
        \only<1>{
        \item Quantify where we expect $\mathcal{D}(\bm{\theta})$ small and highly
              unknown with Bayesian acquisition function $A,$ and choose
              $\argmin_{\bm{\theta}}A(\bm\theta)$}
              \only<2>{
        \item \cite{gutmann_bayesian_2016} use lower confidence bound
              $$A_\text{LCB}(\bm\theta) := \mu(\bm\theta) - \eta_t\sqrt{\mathrm{v}(\bm\theta)}$$ \begin{itemize}
                  \item $\mu(\bm\theta),$
                        $\mathrm{v}(\bm\theta)$ are posterior mean and variance of
                        $D_\mathcal{GP}(\bm{\theta})$
              ...
              \begin{align*}
                  A_\text{EI}(\bm\theta)
                  := & \E[\min(\mu_\text{min} - \mathcal{D}_\mathcal{GP}(\bm\theta), 0)] % \\
...
\begin{frame}
    ...
    \only<5>{\includegraphics[width=\textwidth]{
            ../../champagne_GP_images/lambda_slice_200_synth_likelihood.pdf}}
    \only<6>{\includegraphics[width=\textwidth]{
            ../../champagne_GP_images/initial_f_slice_log_discrep.pdf}}
    \only<7>{\includegraphics[width=\textwidth]{
            ../../champagne_GP_images/f_slice_200_bolfi_updates_log_discrep.pdf}}
    \only<8>{\includegraphics[width=\textwidth]{
            ../../champagne_GP_images/f_slice_200_synth_likelihood.pdf}}
\end{frame}
